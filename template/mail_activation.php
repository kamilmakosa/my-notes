...
if ($_SERVER['SCRIPT_NAME'] == '/template/mail_activation.php') {
	session_start();
...
<div id="content">
	...
	<div class="window" id="window-registration">
		...
		<p style="font-size: 16px;">Otrzymałeś tę wiadomość, ponieważ zarejestrowałeś na ten adres email konto w serwisie <?php echo mb_convert_case(get_domain('http://'.$_SERVER['SERVER_NAME']),MB_CASE_TITLE, "UTF-8"); ?> (jeżeli nie zakładałeś konta - możesz zignorować tę wiadomość, a na ten email nie powinieneś otrzymać więcej żadnych niepożądanych wiadomości).</p>
		<p style="font-size: 16px; font-weight: bold; ">Aby aktywować konto wystarczy, że <a href="<?php echo 'http://'.$_SERVER['HTTP_HOST'].'/activate/?user='.@$name.'&key='.@$user_activation_key; ?>">klikniesz tutaj</a>!</p>
		<p>Możesz także wypełnić formularz aktywacyjny, który jest dostępny <a href="<?php echo 'http://'.$_SERVER['HTTP_HOST'].'/activate/'; ?>">pod tym linkiem</a>.</p>
		<p style="font-size: 16px;"><span style="font-weight: bold;">Klucz aktywacyjny:</span> <span style="font-style: italic;"><?php echo @$user_activation_key?></span></p>
		<p>--<br/>
		Pozdrawiamy,<br/>
