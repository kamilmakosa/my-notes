...
if (isset($operation)) {
	...
	if ($operation == 'down' && $position != $rowcount) {
		$position2 = $position+1;
		$query = "UPDATE `ms_notes` SET notes_position='$user_login' WHERE notes_owner='$user_login' AND notes_position='$position'";
		$result = mysqli_query($con,$query);
		$query = "UPDATE `ms_notes` SET notes_position='$position' WHERE notes_owner='$user_login' AND notes_position='$position2'";
		$result = mysqli_query($con,$query);
		$query = "UPDATE `ms_notes` SET notes_position='$position2' WHERE notes_owner='$user_login' AND notes_position='$user_login'";
		$result = mysqli_query($con,$query);
		$_SESSION['notes_alert'] = alert('success','Przesunięto notatkę.');
		header('Location: /notes/');
	}
...
?>
		<div class="window table-list" id="window-notes">
			...
			<div id="table-searcher">
				...
				<span class="right" title="<?php echo __('Version'); ?>: Standard"><?php echo icon('view_list','/notes'); ?></span>
			</div>
...
<?php } ?>
				...
				<tfoot>
					<tr>
						<td colspan="8"><a href="bookmarks/new"><?php echo __('Add new note'); ?></a></td>
					</tr>
				</tfoot>
			</table></div>
<?php
